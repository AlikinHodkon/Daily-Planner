  ...
  return (
    <div className="w-[15%] flex flex-col border-black border-[2px] min-h-[100vh]">
        ...
        <div className="flex w-full mt-2">
            <input type="text" className="w-3/4 pl-2 rounded-xl border-black border-[2px]" onChange={(e) => setTask(e.target.value)} placeholder="Дело" />
            <button className="w-1/3" onClick={() => {addTask(task, day.date, day.time);}}>Добавить</button>
        </div>
    </div>
  )
}
